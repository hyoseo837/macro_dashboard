...
## Current state — v4 (in progress, deployed)

- **News widgets**: RSS headline feeds from 22 sources (BBC, CNN, Reuters, NYT, HN, Korea Herald, CBC). Pick a feed, get live headlines. Hourly refresh, 7-day retention.
...
- **News widgets**: RSS headlines from curated feeds — responsive article count by widget size
- **Add/edit/delete widgets** via modals in edit mode
- **Yahoo Finance autocomplete** for adding new assets
- Backend scheduler refreshes prices every 15 min, news every 60 min; frontend polls prices every 60s
- Dark mode only

## Stack

- **Frontend**: Vite + React 19 + TypeScript + Tailwind CSS v4 + React Query + Recharts + react-grid-layout + react-router-dom
- **Backend**: FastAPI + SQLAlchemy 2.x (async) + PostgreSQL 16 (Docker) + APScheduler + yfinance + feedparser + python-jose (JWT) + bcrypt + aiosmtplib
...
| **v4** | News Widgets | in progress |
| **v4B** | AI News (Gemini) | planned |
