...
- ~~add widget fails but few seconds later it suddenly appears.~~ — root cause was `needs_fetch` uninitialized for asset/time widgets causing 500 after commit. Fixed + `setQueryData` for instant cache update + `onSettled` fallback
...
- ~~Asset widget AI summary~~ — Gemini generates one-liner explaining weekly price movement using news context. Stored in `price_snapshots.summary`, refreshed hourly via AI pipeline. Displayed at the bottom of all widget sizes as a marquee ticker with divider.
